...
exports.registerMember = async (req, res) => {
    try {
        ...
        if (memberShip) {
            ...
            res.status(200).json({ error: 'Member Registered Successfully', newmember });
        } else {
...
exports.expiringWithin3Days = async (req, res) => {
    try {
        ...
        res.status(200).json({
            message: member.length ? "Fetched Members Successfully" : "No Such Member is Expiring within",
            members: member,
            totalMembers: member.length
        })
